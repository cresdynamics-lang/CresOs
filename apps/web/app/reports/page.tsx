    ...
    <section className="flex min-h-0 w-full min-w-0 flex-1 flex-col gap-5 px-3 py-4 sm:px-6 sm:py-5">
        ...
        actions={
          <>
            {isLeadership && (
              <Link
                href="/reports/ai"
                className="inline-flex shrink-0 items-center justify-center rounded-xl border border-violet-500/40 bg-[#F9F0FB] px-4 py-2.5 text-sm font-medium text-violet-200 hover:bg-[#F9F0FB]"
              >
                AI Reports →
              </Link>
            )}
      ...
      {!isLeadership && overdue.length > 0 && (
        <div className="shrink-0 rounded-2xl border border-amber-500/40 bg-gradient-to-br from-amber-950/50 via-slate-950/90 to-slate-950 px-4 py-3 sm:px-5">
          ...
          <ul className="mt-2 list-inside list-disc text-sm text-[#5B6472]">
            {overdue.slice(0, 5).map((o) => (
              <li key={o.id}>
                <Link href={`/reports/${o.reportId}`} className="text-[#B45309] hover:underline">
                  {o.reportTitle}
                </Link>
                {" — answer by deadline"}
              </li>
            ))}
            {overdue.length > 5 && <li className="text-[#5B6472]">… and {overdue.length - 5} more</li>}
          </ul>
        </div>
      )}
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          {reports.length === 0 ? (
            <div className="flex min-h-[16rem] flex-col items-center justify-center rounded-xl border border-dashed border-amber-800/40 bg-gradient-to-br from-amber-950/25 via-slate-950/60 to-slate-950 px-6 py-12 text-center">
              <p className="font-display text-xl font-bold tracking-tight text-[#92400E]/90">
              ...
              <table className="min-w-full text-left text-sm text-[#1A1D26]">
                ...
                <tbody>
                  {reports.map((r) => (
                    <tr
                      key={r.id}
                      className="border-b border-[#E5E9EF]/60 align-top transition-colors hover:bg-amber-500/5"
                    >
                      <td className="px-3 py-2.5">
                        <Link href={`/reports/${r.id}`} className="font-medium text-[#B45309] hover:underline">
                          {r.title}
                        </Link>
                      </td>
                      {isLeadership && (
                        <td className="px-3 py-2.5 text-xs text-[#5B6472]">
                          {r.submittedBy ? r.submittedBy.name ?? r.submittedBy.email : "—"}
                        </td>
                      )}
                      {isLeadership && (
                        <td
                          className="max-w-[14rem] px-3 py-2.5 text-xs text-[#5B6472]"
                      ...
                      {isLeadership && (
                        <td className="px-3 py-2.5 text-xs tabular-nums text-[#5B6472]">
                      ...
                      {isLeadership && (
                        <td className="px-3 py-2.5 text-xs">
                          {r.hasAiLeadershipReply === true ? (
                            <span className="rounded-full bg-sky-500/15 px-2 py-0.5 text-[#2563EB]">Yes</span>
                          ) : r.hasAiLeadershipReply === false ? (
                            <span className="text-[#5B6472]">No</span>
                          ) : (
                            "—"
                          )}
                        </td>
                      ...
                      <td className="px-3 py-2.5 text-xs">
                        <span
                          className={
                            r.status === "submitted"
                              ? "rounded-full bg-emerald-500/15 px-2 py-0.5 text-[#1B6B3A]"
                              : "rounded-full bg-slate-700/80 px-2 py-0.5 text-[#5B6472]"
                          }
                        >
                          {r.status}
                        </span>
                          ...
                          className={
                            r.reviewStatus === "checked"
                              ...
                              : r.reviewStatus === "viewed"
                                ? "rounded-full bg-sky-500/15 px-2 py-0.5 text-[#2563EB]"
                                : "rounded-full bg-amber-500/15 px-2 py-0.5 text-[#92400E]"
                          }
                        >
                          {r.reviewStatus ?? "pending"}
                        </span>
                      </td>
                      <td className="max-w-[10rem] px-3 py-2.5 text-xs text-[#5B6472]">
                        {r.remarks?.trim() ? r.remarks.trim().slice(0, 80) : "—"}
                      </td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-xs text-[#5B6472]">
