...
type NavItem = {
  ...
  label: string;
  shortLabel?: string;
  group: "workspace" | "pipeline" | "delivery" | "review";
  roles: string[];
  match: "exact" | "prefix";
};
...
const GROUP_ORDER: NavItem["group"][] = ["workspace", "pipeline", "delivery", "review"];

export function navForSalesRoles(roleKeys: string[]): NavItem[] {
  return SALES_NAV_ITEMS.filter((item) => item.roles.some((r) => roleKeys.includes(r)));
}
