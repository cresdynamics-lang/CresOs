    ...
    <label className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="h-9 w-10 cursor-pointer rounded-lg border border-sky-200 bg-transparent"
        />
        <input
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="min-w-0 flex-1 rounded-lg border border-sky-200 bg-white px-2.5 py-2 font-mono text-xs text-slate-700"
        />
      </div>
    </label>
  );
...
function ImageUploadField({
  ...
  return (
    <div className="rounded-xl border border-sky-100 bg-[#f5f9fc] p-3">
      <FieldLabel>{label}</FieldLabel>
      <p className="mt-1 text-[11px] text-slate-500">{hint}</p>
      {preview ? (
        <div className="mt-3 overflow-hidden rounded-lg border border-sky-200 bg-white/5">
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-white/90 backdrop-blur-md">
      <div className="flex min-h-0 w-full flex-col">
        <header className="flex shrink-0 items-center justify-between border-b border-sky-100 bg-[#eef4fb] px-5 py-4">
          <div>
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          <aside className="flex w-52 shrink-0 flex-col gap-1 overflow-y-auto border-r border-sky-100 bg-[#eef4fb] p-3">
          ...
          <section className="flex min-w-0 flex-1 flex-col border-r border-sky-100 bg-[#eef4fb]">
            {selected ? (
              <>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {DESIGN_PRESETS.map((p) => (
                      <button
                        ...
                        className="rounded-full border border-sky-200 bg-white px-3 py-1 text-[11px] text-slate-600 hover:border-brand/30 hover:text-brand"
                      >
                        {p.label}
                      </button>
                    ))}
                ...
                <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-sky-100 px-3 py-2">
                  {tabs.map((t) => (
                    <button
                      key={t.id}
                      type="button"
                    ...
                    <div className="grid gap-4">
                      ...
                      <label className="flex flex-col gap-1.5">
                        ...
                        <textarea
                          ...
                          className="rounded-lg border border-sky-200 bg-white px-3 py-2 font-mono text-xs text-slate-700"
                        />
                      </label>
                      <div className="rounded-xl border border-sky-100 bg-[#f5f9fc] p-3">
                        <label className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <div className="grid gap-4">
                      ...
                      {useCustomHtml ? (
                        <textarea
                          ...
                          className="rounded-lg border border-sky-200 bg-white px-3 py-2 font-mono text-xs leading-relaxed text-slate-700"
                        />
                      ) : (
                        <label className="flex flex-col gap-1.5">
                ...
                {message ? (
                  <footer className="shrink-0 border-t border-sky-100 px-4 py-2">
