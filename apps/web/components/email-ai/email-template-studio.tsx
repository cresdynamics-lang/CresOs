    ...
    <label className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="h-9 w-10 cursor-pointer rounded-lg border border-[#E5E9EF] bg-transparent"
        />
        <input
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="min-w-0 flex-1 rounded-lg border border-[#E5E9EF] bg-white px-2.5 py-2 font-mono text-xs text-slate-700"
        />
      </div>
    </label>
  );
...
function ImageUploadField({
  ...
  return (
    <div className="rounded-xl border border-[#E5E9EF] bg-[#F4F7F9] p-3">
      <FieldLabel>{label}</FieldLabel>
      <p className="mt-1 text-[11px] text-slate-500">{hint}</p>
      {preview ? (
        <div className="mt-3 overflow-hidden rounded-lg border border-[#E5E9EF] bg-white/5">
...
export function EmailTemplateStudio({
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-white/90 backdrop-blur-md">
      <div className="flex min-h-0 w-full flex-col">
        <header className="flex shrink-0 items-center justify-between border-b border-[#E5E9EF] bg-[#F4F7F9] px-5 py-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-brand">Email AI Studio</p>
            <h2 className="text-xl font-semibold text-slate-900">Craft your email templates</h2>
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          <aside className="flex w-52 shrink-0 flex-col gap-1 overflow-y-auto border-r border-[#E5E9EF] bg-[#F4F7F9] p-3">
          ...
          <section className="flex min-w-0 flex-1 flex-col border-r border-[#E5E9EF] bg-[#F4F7F9]">
            {selected ? (
              <>
                <div className="shrink-0 border-b border-[#E5E9EF] px-4 py-3">
                  <p className="text-sm text-slate-600">{selected.description}</p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {selected.variables.map((v) => (
                      <code
                        key={v}
                        className="rounded-md bg-brand/10 px-1.5 py-0.5 text-[10px] text-brand ring-1 ring-indigo-500/20"
                      >
                        {`{{${v}}}`}
                      </code>
                    ))}
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {DESIGN_PRESETS.map((p) => (
                      <button
                        ...
                        className="rounded-full border border-[#E5E9EF] bg-white px-3 py-1 text-[11px] text-slate-600 hover:border-brand/30 hover:text-brand"
                      >
                        {p.label}
                      </button>
                    ))}
                ...
                <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-[#E5E9EF] px-3 py-2">
                  {tabs.map((t) => (
                    <button
                      key={t.id}
                      type="button"
                      ...
                      className={`shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
                        tab === t.id
                          ? "bg-brand/10 text-brand ring-1 ring-indigo-500/30"
                          : "text-slate-500 hover:bg-brand/5 hover:text-slate-600"
                      }`}
                    >
                      {t.label}
                    </button>
                    ...
                    <div className="grid gap-4">
                      <label className="flex flex-col gap-1.5">
                        ...
                        <input
                          ...
                          className="rounded-lg border border-[#E5E9EF] bg-white px-3 py-2 text-sm text-slate-700"
                        />
                      </label>
                      <label className="flex flex-col gap-1.5">
                      ...
                      <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
                        <input
                          ...
                          className="rounded border-[#E5E9EF]"
                        />
                        Show logo
                      </label>
                      <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
                    ...
                    <div className="grid gap-4">
                      ...
                      <label className="flex flex-col gap-1.5">
                        ...
                        <textarea
                          ...
                          className="rounded-lg border border-[#E5E9EF] bg-white px-3 py-2 font-mono text-xs text-slate-700"
                        />
                      </label>
                      <div className="rounded-xl border border-[#E5E9EF] bg-[#F4F7F9] p-3">
                    ...
                    <div className="grid gap-4">
                      ...
                      {useCustomHtml ? (
                        <textarea
                          ...
                          className="rounded-lg border border-[#E5E9EF] bg-white px-3 py-2 font-mono text-xs leading-relaxed text-slate-700"
                        />
                      ) : (
                        <label className="flex flex-col gap-1.5">
                ...
                {message ? (
                  <footer className="shrink-0 border-t border-[#E5E9EF] px-4 py-2">
