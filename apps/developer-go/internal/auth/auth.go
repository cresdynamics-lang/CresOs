...
func (s *Service) Login(ctx context.Context, email, password, ip, ua string) (token string, user User, err error) {
	...
	if !HasDeveloperAccess(roles) {
		return "", User{}, errors.New("developer workspace access required")
	}
...
func HasDeveloperAccess(roles []string) bool {
	for _, r := range roles {
		if r == "developer" || r == "admin" || r == "director_admin" || r == "project_manager" {
			return true
		}
	}
	return false
...
func IsLeadership(roles []string) bool {
	return HasRole(roles, "admin") || HasRole(roles, "director_admin") || HasRole(roles, "project_manager")
}
...
func Initials(name, email string) string {
	...
	return strings.ToUpper(local[:1])
}
