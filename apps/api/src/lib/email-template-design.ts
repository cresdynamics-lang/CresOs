/** Visual design tokens for org email templates (compiled to table-safe HTML). */
...
/** Turn relative /uploads paths into absolute URLs for email clients. */
...
export function compileTemplateHtml(
  ...
  opts?: { publicApiBase?: string }
): string {
  const base = (opts?.publicApiBase ?? process.env.PUBLIC_API_URL ?? process.env.APP_URL ?? "http://localhost:4000").replace(
    /\/+$/,
    ""
  );
